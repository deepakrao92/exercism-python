...
def letter_grades(highest):
    ...
    MIN_PASSING_GRADE = 40
    grades = []
    interval_size = (highest - MIN_PASSING_GRADE) // 4
    
    for loop_ctr in range(0, 4):
        grades.append((MIN_PASSING_GRADE + 1)  + interval_size * loop_ctr)
...
def student_ranking(student_scores, student_names):
    ...
    for rank, score in enumerate(student_scores):
        ranked_students.append(
            f'{rank + 1}. {student_names[rank]}: {score}'
            )
...
def perfect_score(student_info):
    ...
    for student_name, score in student_info:
        if score == 100:
            _perfect_score.append(student_name)
            _perfect_score.append(score)
            break
